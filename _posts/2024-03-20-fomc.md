...
*Dev Log: Fedbot is experimenting with a new workflow to auto-label sentiment, relevance, and importance for Fedspeak. My judgement is that the bot performs much better when 1) it is prompted to perform comparison and has access to some historical context/previous benchmark and 2) reduce "randomness" by adjusting model parameters, so that's what we are trying to achieve. The interim result you are seeing is a "zero-shot" result: it is a score based on the text itself, and it's pretty useless.

*Another issue we need to consider here is that the sentiment could change between 1)assessing the sentence alone and 2) assessing a few sentences as a whole. Currently the approach could be, when a sentence has a pretty different sentiment compared to its neighbor sentences, reassess the whole paragraph? havent tried it yet.*
...
#### Q&A 
<p>
Q: Thank you. A mic for me. Steve Liesman, CNBC. Mr. Chairman, the projections show somewhat higher core inflation.They also show somewhat stronger growth. What should we infer from this notion that on average rates were kept the same this year but inflation is higher and growth is higher? Does it mean more tolerance for higher inflation and less of a willingness to slow the economy to achieve that target?
